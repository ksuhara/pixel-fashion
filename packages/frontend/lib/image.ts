...
export class Image {
   ...
   */
  public toRLE(getRgbaAt: (x: number, y: number) => RGBAColor, colors: Map<string, number>) {
    if (!this._rle) {
   ...
   * @param colors The color palette map
   */
  private encode(getRgbaAt: (x: number, y: number) => RGBAColor, colors: Map<string, number>) {
    const hexColors = [];
  ...
  private updateImageBounds(y: number): void {
    ...
    if (!this.isEmptyRow(rects[0]) && this._bounds.top === 0 && !this._isStartTop) {
    ...
    this._rows[y].bounds = {
      ...
      right: rects[0].length === this._width ? this._width : this._width - rects[rects.length - 1].length,
    };
  }
